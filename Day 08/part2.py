...
def apply_antinodes(city, nodes):
    ...
    for node in nodes:
        for pos_1, pos_2 in combinations(nodes[node], 2):
            ...
            while not (antinode_1_i < 0 or antinode_1_i >= height or antinode_1_j < 0 or antinode_1_j >= width):
                city[antinode_1_i][antinode_1_j] = "#"
                antinode_1_i, antinode_1_j = antinode_1_i - delta_i, antinode_1_j - delta_j

            antinode_2_i, antinode_2_j = i_2 + delta_i, j_2 + delta_j
            while not (antinode_2_i < 0 or antinode_2_i >= height or antinode_2_j < 0 or antinode_2_j >= width):
                city[antinode_2_i][antinode_2_j] = "#"
                antinode_2_i, antinode_2_j = antinode_2_i + delta_i, antinode_2_j + delta_j

def count_all_antinodes(city):
    s = 0
    for line in city:
        for node in line:
            if node != ".":
...
print(count_all_antinodes(city))