...
heap = []
hp.heappush(heap, (0, *startpos, 1, [startpos]))

while len(heap) > 0:
    score, i, j, og_dir, path = hp.heappop(heap)
    if not score > bestendscore:
        bestscores[i][j][og_dir] = score    
        if (i, j) == endpos:
            bestendscore = score
            bestendpaths.update(path)
                ...
                match lab[new_i][new_j]:
                    ...
                    case _:
                        if k == og_dir and bestscores[new_i][new_j][k] > score + 1:
                            hp.heappush(heap, (score + 1, new_i, new_j, k, path + [(new_i, new_j)]))
                        elif bestscores[new_i][new_j][k] > score + 1000:
                            hp.heappush(heap, (score + 1000, i, j, k, path))

print(len(bestendpaths))